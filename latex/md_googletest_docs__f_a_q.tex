If you cannot find the answer to your question here, and you have read \mbox{\hyperlink{_primer_8md}{Primer}} and \mbox{\hyperlink{_advanced_guide_8md}{Advanced\+Guide}}, send it to \href{mailto:googletestframework@googlegroups.com}{\texttt{ googletestframework@googlegroups.\+com}}.\hypertarget{md_googletest_docs__f_a_q_autotoc_md77}{}\doxysubsection{Why should I use Google Test instead of my favorite C++ testing framework?}\label{md_googletest_docs__f_a_q_autotoc_md77}
...
\end{DoxyItemize}
\end{DoxyItemize}\hypertarget{md_googletest_docs__f_a_q_autotoc_md78}{}\doxysubsection{I\textquotesingle{}m getting warnings when compiling Google Test.  Would you fix them?}\label{md_googletest_docs__f_a_q_autotoc_md78}
...
If you violate the rule, there may not be immediately consequences, but your test may (just may) break with a new compiler (or a new version of the compiler you are using) or with a new version of Google Test. Therefore it\textquotesingle{}s best to follow the rule.\hypertarget{md_googletest_docs__f_a_q_autotoc_md80}{}\doxysubsection{Why is it not recommended to install a pre-\/compiled copy of Google Test (for example, into /usr/local)?}\label{md_googletest_docs__f_a_q_autotoc_md80}
...
In order to prevent build output files from overwriting one another, you\textquotesingle{}ll need to change the {\ttfamily Intermediate Directory} settings for the newly created platform configuration across all the projects. To do this, multi-\/select (e.\+g. using shift-\/click) all projects (but not the solution) in the {\ttfamily Solution Explorer}. Right-\/click one of them and select {\ttfamily Properties}. In the left pane, select {\ttfamily Configuration Properties}, and from the {\ttfamily Configuration} dropdown, select {\ttfamily All Configurations}. Make sure the selected platform is {\ttfamily x64}. For the {\ttfamily Intermediate Directory} setting, change the value from {\ttfamily \textbackslash{}} to {\ttfamily \textbackslash{}}. Click {\ttfamily OK} and then build the solution. When the build is complete, the 64-\/bit binaries will be in the {\ttfamily msvc\textbackslash{}x64\textbackslash{}Debug} directory.\hypertarget{md_googletest_docs__f_a_q_autotoc_md82}{}\doxysubsection{Can I use Google Test on Min\+G\+W?}\label{md_googletest_docs__f_a_q_autotoc_md82}
...
Please contact {\ttfamily googletestframework@googlegroups.\+com} if you are interested in improving the support for Min\+GW.\hypertarget{md_googletest_docs__f_a_q_autotoc_md83}{}\doxysubsection{Why does Google Test support E\+X\+P\+E\+C\+T\+\_\+\+E\+Q(\+N\+U\+L\+L, ptr) and A\+S\+S\+E\+R\+T\+\_\+\+E\+Q(\+N\+U\+L\+L, ptr) but not E\+X\+P\+E\+C\+T\+\_\+\+N\+E(\+N\+U\+L\+L, ptr) and A\+S\+S\+E\+R\+T\+\_\+\+N\+E(\+N\+U\+L\+L, ptr)?}\label{md_googletest_docs__f_a_q_autotoc_md83}
...
Finally, with the growth of Google Mock\textquotesingle{}s \href{../../googlemock/docs/CookBook.md\#using-matchers-in-google-test-assertions}{\texttt{ matcher}} library, we are encouraging people to use the unified {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+T\+H\+A\+T(value, matcher)} syntax more often in tests. One significant advantage of the matcher approach is that matchers can be easily combined to form new matchers, while the {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+NE}, etc, macros cannot be easily combined. Therefore we want to invest more in the matchers than in the {\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+X\+X()} macros.\hypertarget{md_googletest_docs__f_a_q_autotoc_md84}{}\doxysubsection{Does Google Test support running tests in parallel?}\label{md_googletest_docs__f_a_q_autotoc_md84}
Test runners tend to be tightly coupled with the build/test environment, and Google Test doesn\textquotesingle{}t try to solve the problem of running tests in parallel. Instead, we tried to make Google Test work nicely with test runners. For example, Google Test\textquotesingle{}s X\+ML report contains the time spent on each test, and its {\ttfamily gtest\+\_\+list\+\_\+tests} and {\ttfamily gtest\+\_\+filter} flags can be used for splitting the execution of test methods into multiple processes. These functionalities can help the test runner run the tests in parallel.\hypertarget{md_googletest_docs__f_a_q_autotoc_md85}{}\doxysubsection{Why don\textquotesingle{}t Google Test run the tests in different threads to speed things up?}\label{md_googletest_docs__f_a_q_autotoc_md85}
...
If you think about it, it\textquotesingle{}s already hard to make your code work when you know what other threads are doing. It\textquotesingle{}s much harder, and sometimes even impossible, to make your code work when you don\textquotesingle{}t know what other threads are doing (remember that test methods can be added, deleted, or modified after your test was written). If you want to run the tests in parallel, you\textquotesingle{}d better run them in different processes.\hypertarget{md_googletest_docs__f_a_q_autotoc_md86}{}\doxysubsection{Why aren\textquotesingle{}t Google Test assertions implemented using exceptions?}\label{md_googletest_docs__f_a_q_autotoc_md86}
...
The downside of not using exceptions is that {\ttfamily A\+S\+S\+E\+R\+T\+\_\+$\ast$} (implemented using {\ttfamily return}) will only abort the current function, not the current {\ttfamily T\+E\+ST}.\hypertarget{md_googletest_docs__f_a_q_autotoc_md87}{}\doxysubsection{Why do we use two different macros for tests with and without fixtures?}\label{md_googletest_docs__f_a_q_autotoc_md87}
...
We think neither approach is ideal, yet either of them is reasonable. In the end, it probably doesn\textquotesingle{}t matter much either way.\hypertarget{md_googletest_docs__f_a_q_autotoc_md88}{}\doxysubsection{Why don\textquotesingle{}t we use structs as test fixtures?}\label{md_googletest_docs__f_a_q_autotoc_md88}
We like to use structs only when representing passive data. This distinction between structs and classes is good for documenting the intent of the code\textquotesingle{}s author. Since test fixtures have logic like {\ttfamily Set\+Up()} and {\ttfamily Tear\+Down()}, they are better defined as classes.\hypertarget{md_googletest_docs__f_a_q_autotoc_md89}{}\doxysubsection{Why are death tests implemented as assertions instead of using a test runner?}\label{md_googletest_docs__f_a_q_autotoc_md89}
...
Another interesting thing about {\ttfamily A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+TH} is that it calls {\ttfamily fork()} to create a child process to run the death test. This is lightening fast, as {\ttfamily fork()} uses copy-\/on-\/write pages and incurs almost zero overhead, and the child process starts from the user-\/supplied statement directly, skipping all global and local initialization and any code leading to the given statement. If you launch the child process from scratch, it can take seconds just to load everything and start running if the test links to many libraries dynamically.\hypertarget{md_googletest_docs__f_a_q_autotoc_md90}{}\doxysubsection{My death test modifies some state, but the change seems lost after the death test finishes. Why?}\label{md_googletest_docs__f_a_q_autotoc_md90}
Death tests ({\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+D\+E\+A\+TH}, etc) are executed in a sub-\/process s.\+t. the expected crash won\textquotesingle{}t kill the test program (i.\+e. the parent process). As a result, any in-\/memory side effects they incur are observable in their respective sub-\/processes, but not in the parent process. You can think of them as running in a parallel universe, more or less.\hypertarget{md_googletest_docs__f_a_q_autotoc_md91}{}\doxysubsection{The compiler complains about \char`\"{}undefined references\char`\"{} to some static const member variables, but I did define them in the class body. What\textquotesingle{}s wrong?}\label{md_googletest_docs__f_a_q_autotoc_md91}
...
Otherwise your code is {\bfseries{invalid C++}}, and may break in unexpected ways. In particular, using it in Google Test comparison assertions ({\ttfamily E\+X\+P\+E\+C\+T\+\_\+\+EQ}, etc) will generate an \char`\"{}undefined reference\char`\"{} linker error.\hypertarget{md_googletest_docs__f_a_q_autotoc_md92}{}\doxysubsection{I have an interface that has several implementations. Can I write a set of tests once and repeat them over all the implementations?}\label{md_googletest_docs__f_a_q_autotoc_md92}
Google Test doesn\textquotesingle{}t yet have good support for this kind of tests, or data-\/driven tests in general. We hope to be able to make improvements in this area soon.\hypertarget{md_googletest_docs__f_a_q_autotoc_md93}{}\doxysubsection{Can I derive a test fixture from another?}\label{md_googletest_docs__f_a_q_autotoc_md93}
Yes.
...
For a complete example using derived test fixtures, see \href{../samples/sample5_unittest.cc}{\texttt{ sample5}}.\hypertarget{md_googletest_docs__f_a_q_autotoc_md94}{}\doxysubsection{My compiler complains \char`\"{}void value not ignored as it ought to be.\char`\"{} What does this mean?}\label{md_googletest_docs__f_a_q_autotoc_md94}
You\textquotesingle{}re probably using an {\ttfamily A\+S\+S\+E\+R\+T\+\_\+$\ast$()} in a function that doesn\textquotesingle{}t return {\ttfamily void}. {\ttfamily A\+S\+S\+E\+R\+T\+\_\+$\ast$()} can only be used in {\ttfamily void} functions.\hypertarget{md_googletest_docs__f_a_q_autotoc_md95}{}\doxysubsection{My death test hangs (or seg-\/faults). How do I fix it?}\label{md_googletest_docs__f_a_q_autotoc_md95}
...
\end{DoxyItemize}\hypertarget{md_googletest_docs__f_a_q_autotoc_md97}{}\doxysubsection{The compiler complains \char`\"{}no matching function to call\char`\"{} when I use A\+S\+S\+E\+R\+T\+\_\+\+P\+R\+E\+Dn. How do I fix it?}\label{md_googletest_docs__f_a_q_autotoc_md97}
...
\DoxyCodeLine{ASSERT\_PRED2(*(GreaterThan<int, int>)*, 5, 0);}
\end{DoxyCode}
\hypertarget{md_googletest_docs__f_a_q_autotoc_md98}{}\doxysubsection{My compiler complains about \char`\"{}ignoring return value\char`\"{} when I call R\+U\+N\+\_\+\+A\+L\+L\+\_\+\+T\+E\+S\+T\+S(). Why?}\label{md_googletest_docs__f_a_q_autotoc_md98}
...
This is wrong and dangerous. A test runner needs to see the return value of {\ttfamily \mbox{\hyperlink{gtest_8h_a853a3792807489591d3d4a2f2ff9359f}{R\+U\+N\+\_\+\+A\+L\+L\+\_\+\+T\+E\+S\+T\+S()}}} in order to determine if a test has passed. If your {\ttfamily \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}}} function ignores it, your test will be considered successful even if it has a Google Test assertion failure. Very bad.

To help the users avoid this dangerous bug, the implementation of {\ttfamily \mbox{\hyperlink{gtest_8h_a853a3792807489591d3d4a2f2ff9359f}{R\+U\+N\+\_\+\+A\+L\+L\+\_\+\+T\+E\+S\+T\+S()}}} causes gcc to raise this warning, when the return value is ignored. If you see this warning, the fix is simple\+: just make sure its value is used as the return value of {\ttfamily \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}}}.\hypertarget{md_googletest_docs__f_a_q_autotoc_md99}{}\doxysubsection{My compiler complains that a constructor (or destructor) cannot return a value. What\textquotesingle{}s going on?}\label{md_googletest_docs__f_a_q_autotoc_md99}
...
we had to give up using {\ttfamily A\+S\+S\+E\+R\+T$\ast$} and {\ttfamily F\+A\+I\+L$\ast$} (but not {\ttfamily E\+X\+P\+E\+C\+T$\ast$} and {\ttfamily A\+D\+D\+\_\+\+F\+A\+I\+L\+U\+R\+E$\ast$}) in constructors and destructors. The workaround is to move the content of your constructor/destructor to a private void member function, or switch to {\ttfamily E\+X\+P\+E\+C\+T\+\_\+$\ast$()} if that works. This section in the user\textquotesingle{}s guide explains it.\hypertarget{md_googletest_docs__f_a_q_autotoc_md100}{}\doxysubsection{My set-\/up function is not called. Why?}\label{md_googletest_docs__f_a_q_autotoc_md100}
C++ is case-\/sensitive. It should be spelled as {\ttfamily Set\+Up()}. Did you spell it as {\ttfamily Setup()}?

Similarly, sometimes people spell {\ttfamily Set\+Up\+Test\+Case()} as {\ttfamily Setup\+Test\+Case()} and wonder why it\textquotesingle{}s never called.\hypertarget{md_googletest_docs__f_a_q_autotoc_md101}{}\doxysubsection{How do I jump to the line of a failure in Emacs directly?}\label{md_googletest_docs__f_a_q_autotoc_md101}
Google Test\textquotesingle{}s failure message format is understood by Emacs and many other I\+D\+Es, like acme and X\+Code. If a Google Test message is in a compilation buffer in Emacs, then it\textquotesingle{}s clickable. You can now hit {\ttfamily enter} on a message to jump to the corresponding source code, or use {\ttfamily C-\/x}\`{} to jump to the next failure.\hypertarget{md_googletest_docs__f_a_q_autotoc_md102}{}\doxysubsection{I have several test cases which share the same test fixture logic, do I have to define a new test fixture class for each of them? This seems pretty tedious.}\label{md_googletest_docs__f_a_q_autotoc_md102}
...
\DoxyCodeLine{TEST\_F(BarTest, Def) \{ ... \}}
\end{DoxyCode}
\hypertarget{md_googletest_docs__f_a_q_autotoc_md103}{}\doxysubsection{The Google Test output is buried in a whole bunch of log messages. What do I do?}\label{md_googletest_docs__f_a_q_autotoc_md103}
...
\end{DoxyCode}
\hypertarget{md_googletest_docs__f_a_q_autotoc_md104}{}\doxysubsection{Why should I prefer test fixtures over global variables?}\label{md_googletest_docs__f_a_q_autotoc_md104}
...
\end{DoxyEnumerate}\hypertarget{md_googletest_docs__f_a_q_autotoc_md105}{}\doxysubsection{How do I test private class members without writing F\+R\+I\+E\+N\+D\+\_\+\+T\+E\+S\+T()s?}\label{md_googletest_docs__f_a_q_autotoc_md105}
...
\end{DoxyItemize}\hypertarget{md_googletest_docs__f_a_q_autotoc_md106}{}\doxysubsection{How do I test private class static members without writing F\+R\+I\+E\+N\+D\+\_\+\+T\+E\+S\+T()s?}\label{md_googletest_docs__f_a_q_autotoc_md106}
...
\end{DoxyCode}
\hypertarget{md_googletest_docs__f_a_q_autotoc_md107}{}\doxysubsection{I would like to run a test several times with different parameters. Do I need to write several similar copies of it?}\label{md_googletest_docs__f_a_q_autotoc_md107}
No. You can use a feature called \href{AdvancedGuide.md\#Value_Parameterized_Tests}{\texttt{ value-\/parameterized tests}} which lets you repeat your tests with different parameters, without defining it more than once.\hypertarget{md_googletest_docs__f_a_q_autotoc_md108}{}\doxysubsection{How do I test a file that defines main()?}\label{md_googletest_docs__f_a_q_autotoc_md108}
To test a {\ttfamily foo.\+cc} file, you need to compile and link it into your unit test program. However, when the file contains a definition for the {\ttfamily \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}}} function, it will clash with the {\ttfamily \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}}} of your unit test, and will result in a build error.
...
\end{DoxyEnumerate}
\begin{DoxyEnumerate}
\item {\ttfamily foo.\+cc} which contains the definitions except {\ttfamily \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}}}, and
\end{DoxyEnumerate}
\begin{DoxyEnumerate}
\item {\ttfamily foo\+\_\+main.\+cc} which contains nothing but the definition of {\ttfamily \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}}}.
\end{DoxyEnumerate}
...
However, please remember this is a hack and should only be used as the last resort.\hypertarget{md_googletest_docs__f_a_q_autotoc_md109}{}\doxysubsection{What can the statement argument in A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H() be?}\label{md_googletest_docs__f_a_q_autotoc_md109}
...
{\ttfamily googletest\+\_\+unittest.\+cc} contains more examples if you are interested.\hypertarget{md_googletest_docs__f_a_q_autotoc_md110}{}\doxysubsection{What syntax does the regular expression in A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H use?}\label{md_googletest_docs__f_a_q_autotoc_md110}
On P\+O\+S\+IX systems, Google Test uses the P\+O\+S\+IX Extended regular expression syntax (\href{http://en.wikipedia.org/wiki/Regular_expression\#POSIX_Extended_Regular_Expressions}{\texttt{ http\+://en.\+wikipedia.\+org/wiki/\+Regular\+\_\+expression\#\+P\+O\+S\+I\+X\+\_\+\+Extended\+\_\+\+Regular\+\_\+\+Expressions}}). On Windows, it uses a limited variant of regular expression syntax. For more details, see the \href{AdvancedGuide.md\#Regular_Expression_Syntax}{\texttt{ regular expression syntax}}.\hypertarget{md_googletest_docs__f_a_q_autotoc_md111}{}\doxysubsection{I have a fixture class Foo, but T\+E\+S\+T\+\_\+\+F(\+Foo, Bar) gives me error \char`\"{}no matching function for call to Foo\+::\+Foo()\char`\"{}. Why?}\label{md_googletest_docs__f_a_q_autotoc_md111}
...
\end{DoxyItemize}\hypertarget{md_googletest_docs__f_a_q_autotoc_md112}{}\doxysubsection{Why does A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H complain about previous threads that were already joined?}\label{md_googletest_docs__f_a_q_autotoc_md112}
...
The new N\+P\+TL thread library doesn\textquotesingle{}t suffer from this problem, as it doesn\textquotesingle{}t create a manager thread. However, if you don\textquotesingle{}t control which machine your test runs on, you shouldn\textquotesingle{}t depend on this.\hypertarget{md_googletest_docs__f_a_q_autotoc_md113}{}\doxysubsection{Why does Google Test require the entire test case, instead of individual tests, to be named F\+O\+O\+Death\+Test when it uses A\+S\+S\+E\+R\+T\+\_\+\+D\+E\+A\+T\+H?}\label{md_googletest_docs__f_a_q_autotoc_md113}
...
\end{DoxyCode}
\hypertarget{md_googletest_docs__f_a_q_autotoc_md115}{}\doxysubsection{The compiler complains about \char`\"{}no match for \textquotesingle{}operator$<$$<$\textquotesingle{}\char`\"{} when I use an assertion. What gives?}\label{md_googletest_docs__f_a_q_autotoc_md115}
...
To update this setting open the project properties in the Visual Studio I\+DE then select the branch Configuration Properties $\vert$ C/\+C++ $\vert$ Code Generation and change the option \char`\"{}\+Runtime Library\char`\"{}. You may also try using gtest-\/md.\+vcproj instead of gtest.\+vcproj.\hypertarget{md_googletest_docs__f_a_q_autotoc_md118}{}\doxysubsection{I put my tests in a library and Google Test doesn\textquotesingle{}t run them. What\textquotesingle{}s happening?}\label{md_googletest_docs__f_a_q_autotoc_md118}
Have you read a \href{Primer.md\#important-note-for-visual-c-users}{\texttt{ warning}} on the Google Test Primer page?\hypertarget{md_googletest_docs__f_a_q_autotoc_md119}{}\doxysubsection{I want to use Google Test with Visual Studio but don\textquotesingle{}t know where to start.}\label{md_googletest_docs__f_a_q_autotoc_md119}
Many people are in your position and one of the posted his solution to our mailing list.\hypertarget{md_googletest_docs__f_a_q_autotoc_md120}{}\doxysubsection{I am seeing compile errors mentioning std\+::type\+\_\+traits when I try to use Google Test on Solaris.}\label{md_googletest_docs__f_a_q_autotoc_md120}
...
`export CC=cc C\+XX=CC C\+X\+X\+F\+L\+A\+GS='-\/library=stlport4\textquotesingle{}\`{}\hypertarget{md_googletest_docs__f_a_q_autotoc_md121}{}\doxysubsection{How can my code detect if it is running in a test?}\label{md_googletest_docs__f_a_q_autotoc_md121}
...
However, if you {\itshape really}, {\itshape really}, {\itshape really} have no choice, and if you follow the rule of ending your test program names with {\ttfamily \+\_\+test}, you can use the {\itshape horrible} hack of sniffing your executable name ({\ttfamily argv\mbox{[}0\mbox{]}} in {\ttfamily \mbox{\hyperlink{_c_make_c_compiler_id_8c_a0ddf1224851353fc92bfbff6f499fa97}{main()}}}) to know whether the code is under test.\hypertarget{md_googletest_docs__f_a_q_autotoc_md122}{}\doxysubsection{Google Test defines a macro that clashes with one defined by another library. How do I deal with that?}\label{md_googletest_docs__f_a_q_autotoc_md122}
...
Currently, the following {\ttfamily T\+E\+ST}, {\ttfamily F\+A\+IL}, {\ttfamily S\+U\+C\+C\+E\+ED}, and the basic comparison assertion macros can have alternative names. You can see the full list of covered macros \href{http://www.google.com/codesearch?q=if+!GTEST_DONT_DEFINE_\\w\%2B+package:http://googletest\\.googlecode\\.com+file:/include/gtest/gtest.h}{\texttt{ here}}. More information can be found in the \char`\"{}\+Avoiding Macro Name Clashes\char`\"{} section of the R\+E\+A\+D\+ME file.\hypertarget{md_googletest_docs__f_a_q_autotoc_md123}{}\doxysubsection{Is it O\+K if I have two separate $<$tt$>$\+T\+E\+S\+T(\+Foo, Bar)$<$/tt$>$ test methods defined in different namespaces?}\label{md_googletest_docs__f_a_q_autotoc_md123}
Yes.
...
\DoxyCodeLine{\}  // namespace foo}
\end{DoxyCode}
\hypertarget{md_googletest_docs__f_a_q_autotoc_md124}{}\doxysubsection{How do I build Google Testing Framework with Xcode 4?}\label{md_googletest_docs__f_a_q_autotoc_md124}
If you try to build Google Test\textquotesingle{}s Xcode project with Xcode 4.\+0 or later, you may encounter an error message that looks like \char`\"{}\+Missing S\+D\+K in target gtest\textbackslash{}\+\_\+framework\+: /\+Developer/\+S\+D\+Ks/\+Mac\+O\+S\+X10.\+4u.\+sdk\char`\"{}. That means that Xcode does not support the S\+DK the project is targeting. See the Xcode section in the ../\+R\+E\+A\+D\+ME.md \char`\"{}\+R\+E\+A\+D\+M\+E\char`\"{} file on how to resolve this.\hypertarget{md_googletest_docs__f_a_q_autotoc_md125}{}\doxysubsection{My question is not covered in your F\+A\+Q!}\label{md_googletest_docs__f_a_q_autotoc_md125}
